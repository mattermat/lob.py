...
class Timer:
    """Context manager that logs elapsed time with a label."""
    _order = 0
    ...
    def __exit__(self, *_):
        ...
        Timer._records[self.label] = self._elapsed
        print(f"[{self._elapsed:8.3f}s] {self.label}", flush=True)
        sys.stdout.flush()


def log_time(label, elapsed):
    """Record a time step outside a context manager."""
    Timer._records[label] = elapsed
    print(f"[{elapsed:8.3f}s] {label}", flush=True)
    sys.stdout.flush()


def print_summary():
    """Print a sorted summary of all timings."""
    print("\n" + "=" * 60, flush=True)
    print("Timing summary (sorted by duration)", flush=True)
    print("=" * 60, flush=True)
    total = sum(Timer._records.values())
    for label, elapsed in sorted(Timer._records.items(), key=lambda x: -x[1]):
        pct = elapsed / total * 100
        bar = "█" * int(pct / 2)
        print(f"  {elapsed:8.3f}s ({pct:5.1f}%) {bar} {label}", flush=True)
...
with Timer("data: init TL + from_parquet"):
...
log_time("data: access timestamps", time.perf_counter() - t0)

window = total_duration // 10
custom_buckets = [1, 3, 5, 10]
holding_time_ns = int(1e9)  # 1 second

print(f"\nDataset: {n_ts:,} timestamps, "
      f"{total_duration/1e9:.1f}s total, window={window/1e9:.1f}s", flush=True)

# ═══════════════════════════════════════════════════════════════════════════════
# LOB time-series stats — time each property individually
# ═══════════════════════════════════════════════════════════════════════════════
print("\n--- LOBts properties ---", flush=True)

t0 = time.perf_counter()
spread = tl.lob.spread
log_time("  lob.spread", time.perf_counter() - t0)

t0 = time.perf_counter()
bid = tl.lob.bid
log_time("  lob.bid", time.perf_counter() - t0)

t0 = time.perf_counter()
ask = tl.lob.ask
log_time("  lob.ask", time.perf_counter() - t0)

t0 = time.perf_counter()
midprice = tl.lob.midprice
log_time("  lob.midprice", time.perf_counter() - t0)

t0 = time.perf_counter()
vw_midprice = tl.lob.vw_midprice
log_time("  lob.vw_midprice", time.perf_counter() - t0)

t0 = time.perf_counter()
bidq = tl.lob.bidq
log_time("  lob.bidq", time.perf_counter() - t0)

t0 = time.perf_counter()
askq = tl.lob.askq
log_time("  lob.askq", time.perf_counter() - t0)

t0 = time.perf_counter()
vi = tl.lob.vi
log_time("  lob.vi", time.perf_counter() - t0)

t0 = time.perf_counter()
ofi = tl.lob.order_flow_imbalance
log_time("  lob.order_flow_imbalance", time.perf_counter() - t0)

with Timer("stats: build lob_ts DataFrame"):
    lob_ts = pd.DataFrame({
        'spread':      spread,
        'bid':         bid,
        'ask':         ask,
...
with Timer("summary: merge_asof OHLC"):
...
with Timer("summary: realized_vol 10s + 60s"):
...
# ═══════════════════════════════════════════════════════════════════════════════
# Rolling 5s window: per-step stats
# ═══════════════════════════════════════════════════════════════════════════════
print("\n--- Rolling 5s windows ---", flush=True)
with Timer("rolling 5s: all-compute"):
    records, ts_out = [], []
...
print(f"\nShape: {summary.shape}", flush=True)
