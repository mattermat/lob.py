...
class itch_parser:
    ...
    def messages(self) -> Generator:
        ...
        """
        orders: dict[int, tuple[str, str, float, int]] = (
            {}
        )  # order_ref → (symbol, side, price, shares)
...
def _replace(data: bytes, orders: dict) -> Optional[ReplaceOrder]:
    ...
    return ReplaceOrder(
        "U", ts, orig_ref, new_ref, orig_shares, shares, old_price, price, symbol, side
    )
...
def _execute_price(data: bytes, orders: dict) -> Optional[ExecuteOrder]:
    # type(1) locate(2) tracking(2) ts(6) order_ref(8) exec_shares(4)
    # match(8) printable(1) exec_price(4)
    ts = _parse_ts(data, 5)
    (order_ref,) = struct.unpack_from(">Q", data, 11)
    (exec_shares,) = struct.unpack_from(">I", data, 19)
    (match,) = struct.unpack_from(">Q", data, 23)
